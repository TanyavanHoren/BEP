...
for i=1:set.non.N
    ...
    set.non.site(i).I_max=log(set.laser.power*(set.laser.laser_frame(ceil(set.non.site(i).position_y/set.mic.pixelsize),ceil(set.non.site(i).position_x/set.mic.pixelsize))/max(max(set.laser.laser_frame))));%peak intensity Gauss binding event
