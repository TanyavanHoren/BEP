...
we will need to assess the clustering algorithms. List the filenames in 
a struct such that the datasets can be loaded in easily later. 
...
if generate_new_data.circle==1 
    for m=1:size(av_binding_spots,2)
        for n=1:size(freq_ratio,2)
            ...
            S.set.other.fixed_bind_spots=0; %the number of binding sites is subject to a Poisson distribution
