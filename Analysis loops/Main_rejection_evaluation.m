...
%N_det_freq_ratio = 0.5:0.5:10; %ratio f_specific/f_non_specific
Ndet_freq_ratio = 1:2:9; %ratio f_specific/f_non_specific
...
if correction_binding_spots.circle==1
    for m=1:size(Ndet_binding_spots,2)
        ...
        create_scatter_plot_Nbind(workspaces, Ndet_binding_spots, Ndet_freq_ratio, m);
    end
end
%function that for each dataset chooses correct rejection method 
%function that for each rejection method chooses correct parameters
%function that plots determined N_values versus ratio, for each binding spot number separately
