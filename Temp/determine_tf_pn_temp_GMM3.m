function check = determine_tf_pn_temp_GMM3(ana, i, k, check)
...
for j=1:size(ana.ROI(i).SupResParams,2)
    true_pos_log(j)=sum(ana.ROI(i).SupResParams(j).isOutlier_GMM3==0 && ana.ROI(i).SupResParams(j).isNonSpecific==0); %0(not outlier), 0(specific)
    true_neg_log(j)=sum(ana.ROI(i).SupResParams(j).isOutlier_GMM3~=0 && ana.ROI(i).SupResParams(j).isNonSpecific==1); %1(outlier), 1(non-specific)
