function SupResParams = rainSTORM_fitLocGF_Dion(frameIdx, myFrame,myPixels,bkgdSig,settings)
...
for lpPx = 1:size(myPixels,1) % For local maxima in descending order
    ...
    xx = (1:2*rad+1)';      % x-positions (rows) (pixel widths) as column vector
    yy = (1:2*rad+1)';      % y-positions (cols) (pixel widths) as column vector
    ...
    x0 = myRow;
    sigX = initSig;
    C  = yRows(myRow); % Guess height of f(x). Centre value is a good guess.
...
%         scatter(xfit,4.5)
        ...
        if (residueRows<mintol && abs(x0)<allowX && sigX>allowSig(1) && sigX<allowSig(2))
            break
        end
    end
    residueRows = sum(Beta.^2)/sum(yRows.^2);
    ...
    if (residueRows<tol && abs(x0)<allowX && sigX>allowSig(1) && sigX<allowSig(2))
        fitRowPos = x0-0.5; % Note (-0.5) for image registration
    ...
    if(flagRowFits) % Don't fit col-direction if the row-axis fit was rejected
        ...
        for lpLSF = 1:maxIts
            ...
            Beta = yCols - fofX;
            residueCols = sum(Beta.^2)/sum(yRows.^2);
            if(abs(y0)>allowX || (sigY < allowSig(1)) || (sigY > allowSig(2)) )
                break; % Stop iterating if solution drifts too far
            end
            if (residueCols<mintol)
                break
            end
        end
        residueCols = sum(Beta.^2)/sum(yRows.^2);
