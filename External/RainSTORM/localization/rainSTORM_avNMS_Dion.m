function [myPixels]=rainSTORM_avNMS_Dion(myFrame,rad)
...
avNMSim_base = zeros(size(myFrame,1)+2,size(myFrame,1)+2);
avNMSim_base(2:10,2:10)= (myFrame);
avNMSim= (avNMSim_base);
% avNMSim = myFrame;
...
% avNMSim(1:end-1, :)=avNMSim(1:end-1, :) + avNMSim_base(2:end,:);
% avNMSim(2:end, :)  =avNMSim(2:end, :)   + avNMSim_base(1:end-1,:);
% avNMSim(:, 1:end-1)=avNMSim(:, 1:end-1) + avNMSim_base(:,2:end);
% avNMSim(:, 2:end)  =avNMSim(:, 2:end)   + avNMSim_base(:, 1:end-1);
% avNMSim(1:end-1,1:end-1) = avNMSim(1:end-1,1:end-1) + avNMSim_base(2:end,2:end);
% avNMSim(1:end-1,2:end) = avNMSim(1:end-1,2:end) + avNMSim_base(2:end,1:end-1);
% avNMSim(2:end,1:end-1) = avNMSim(2:end,1:end-1) + avNMSim_base(1:end-1,2:end);
% avNMSim(2:end,2:end) = avNMSim(2:end,2:end) + avNMSim_base(1:end-1,1:end-1);
...
myRR = 2:size(myFrame,1)+1; % Dion edit
myRC = 2:size(myFrame,2)+1; % Dion edit
