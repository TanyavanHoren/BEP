function frame = generate_specific_binding_intensity(object_data, pixel_data, frame, experiment_data)
...
for i=1:object_data.number
    for j=1:object_data.object(i).number_bind
        if object_data.object_bindingspots(i).binding_spot(j).isBound>0
            ...
            center = [xc;yc];
            gauss_data.I_max=poissrnd(2*experiment_data.average_background); %not representative
