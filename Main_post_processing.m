%{
Generate datasets in large amounts and at varying settings, and
test how well the post-processing works:
1) find optimal DBSCAN parameters for each combination of settings
2) test how well the events are classified (specific/non-specific)
3) show the improvement in binding site quantification

INPUTS
-------
none (if no datasets have been generated yet) / datasets

OUTPUTS (most relevant results)
------
optimization_dbscan: Optimal DBSCAN parameters
optimization_method: Optimal clustering algorithm for each combination of settings
bar diagrams of the number of false positives and false negatives
    resulting from (2)
scatter plot showing the improvement in binding site quantification (3)
...
calibration.dbscan = 'calibration_dbscan_21_07_2020'; %which calibration file is used
...
calibration.method = 'calibration_methods_21_07_2020'; %which calibration file is used
...
%% Optimization dbscan - rectangle (1)
...
%% Show improvement in binding spot determination - Circle (3)
...
%% Show improvement in binding spot determination - Rectangle (3)
